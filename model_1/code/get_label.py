...
def get_data(load_path, batch_size):
    ...
    length = len(train2_data)//batch_size
    length_2 = int(length*batch_size)
    train2_data = train2_data[:length_2]
    return train2_data, length
...
train2_data, length = get_data(load_path, batch_size)
wordlist, wordvec =  get_dictionary()
model = load_model('GRU.h5')
label_vec = np.array([0, 1])
label_list = []
for i in tqdm(range(length)):
    inputs = get_inputs_batch(batch_size, i, train2_data, wordlist, wordvec)
...
np.save(os.path.join(load_path,'train2_label'), label_array)
